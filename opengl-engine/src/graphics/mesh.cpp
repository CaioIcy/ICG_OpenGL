...
Renderable* Mesh::InitMesh(unsigned int Index, const aiMesh* paiMesh)
{
    ...
    for (unsigned int i = 0 ; i < paiMesh->mNumVertices ; i++) {
        ...
        VertexData v(pPos->x, pPos->y, pPos->z, 255, 0, 0, 0);
...
bool Mesh::InitMaterials(const aiScene* pScene, const std::string& Filename)
{
    ...
    for (unsigned int i = 0 ; i < pScene->mNumMaterials ; i++) {
        ...
        if (!m_Textures[i]) {
            m_Textures[i] = new Texture(GL_TEXTURE_2D, "assets/images/caio.bmp");
...
void Mesh::Render()
{
    ...
    for (unsigned int i = 0 ; i < m_Entries.size() ; i++) {
        ...
        glVertexAttribPointer(1, 4, GL_FLOAT, GL_FALSE, sizeof(VertexData), (const GLvoid*)(sizeof(float)*3));
