...
bool Mesh::InitMaterials(const aiScene* pScene, const std::string& Filename)
{
    ...
    for (unsigned int i = 0; i< pScene->mNumMaterials; i++){
        ...
        // Load white texture if the model does not have its own
        if (!m_Textures[i]){
            printf("loading white texture\n");
            m_Textures[i] = new Texture(GL_TEXTURE_2D, "assets/images/white.png");
...
void Mesh::Render()
{
    ...
    for (unsigned int i = 0; i < m_Entries.size(); i++){
        ...
        ogle::gllog::CheckGlErrors("4");

        glDrawElements(GL_TRIANGLES, m_Entries[i].NumIndices, GL_UNSIGNED_INT, 0);
    }
    ogle::gllog::CheckGlErrors("5");

    glDisableVertexAttribArray(2);
    glDisableVertexAttribArray(1);
    glDisableVertexAttribArray(0);
    ogle::gllog::CheckGlErrors("6");
}
